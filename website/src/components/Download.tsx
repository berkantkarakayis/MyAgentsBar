...
import { detectMac, DOWNLOAD_LINKS, type MacArch } from '../utils/detectPlatform'

export function Download() {
  const ref = useRef<HTMLDivElement>(null)
  const isInView = useInView(ref, { once: true, margin: '-60px' })
  const [mac, setMac] = useState<{ isMac: boolean; arch: MacArch }>({ isMac: false, arch: 'unknown' })
  const [selected, setSelected] = useState<'apple-silicon' | 'intel' | 'universal'>('universal')
  const [copied, setCopied] = useState(false)

  useEffect(() => {
    const result = detectMac()
    setMac(result)
    if (result.arch === 'apple-silicon') setSelected('apple-silicon')
    else if (result.arch === 'intel') setSelected('intel')
    else setSelected('universal')
  }, [])

  const handleCopy = () => {
    navigator.clipboard.writeText('brew install --cask myagentsbar').then(() => {
      setCopied(true)
  ...
  return (
    <section id="download-section" className="relative py-32 px-6 overflow-hidden" ref={ref}>
      {/* Glow backdrop */}
      <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
        <div className="w-[600px] h-100 bg-violet-600/10 rounded-full blur-3xl" />
        <div className="absolute w-[400px] h-[300px] bg-blue-600/8 rounded-full blur-3xl translate-x-40" />
      </div>
      ...
      <div className="max-w-4xl mx-auto relative">
        ...
        {mac.isMac && (
          ...
          >
            ...
            Detected: {mac.arch === 'apple-silicon' ? 'Apple Silicon Mac' : 'Intel Mac'} — we pre-selected the right build
          </motion.div>
        )}
        ...
        >
          {(Object.entries(DOWNLOAD_LINKS) as [typeof selected, typeof DOWNLOAD_LINKS[keyof typeof DOWNLOAD_LINKS]][]).map(([key, link]) => (
            <motion.button
              key={key}
        ...
        >
          <a href={selectedLink.url} className="w-full sm:w-auto">
            <motion.button
              ...
              style={{ background: 'linear-gradient(135deg, #7c3aed, #3b82f6, #7c3aed)' }}
            >
              <DownloadIcon />
        ...
        >
          <span className="text-xs text-white/30 uppercase tracking-widest">Or via Homebrew (coming soon)</span>
          <motion.button
            onClick={handleCopy}
          ...
          >
            <code className="text-sm text-violet-400 font-mono">brew install --cask myagentsbar</code>
        ...
        >
          {['macOS 14 Sonoma+', 'Free forever', 'MIT License', 'No telemetry'].map(item => (
