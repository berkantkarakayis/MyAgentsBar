...
1) **Cached cookie header** (preferred)
   ...
   - Keychain cache: `com.steipete.myagentsbar.cache` (account `cookie.cursor`).
...
3) **Stored session cookies** (fallback)
   ...
   - Stored at: `~/Library/Application Support/MyAgentsBar/cursor-session.json`.
...
## Key files
- `Sources/MyAgentsBarCore/Providers/Cursor/CursorStatusProbe.swift`
- `Sources/MyAgentsBar/CursorLoginRunner.swift` (login flow)
