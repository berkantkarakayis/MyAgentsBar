...
The original fix (migrating legacy MyAgentsBar keychain items to `kSecAttrAccessibleAfterFirstUnlockThisDeviceOnly`) is
still in place, but the architecture has changed:

- Provider settings and manual secrets are now persisted in `~/.myagentsbar/config.json`.
- Legacy keychain stores are still present mainly to migrate old installs, then clear old items.
- Keychain is still used for runtime cache entries (for example `com.steipete.myagentsbar.cache`) and Claude OAuth
...
| --- | --- |
| MyAgentsBar stores provider credentials only in keychain | Manual/provider settings are config-file backed (`~/.myagentsbar/config.json`), while keychain is still used for runtime caches and Claude OAuth bootstrap fallback. |
| `ClaudeOAuthCredentials.swift` migrated MyAgentsBar-owned Claude OAuth keychain items | Claude OAuth primary source is Claude CLI keychain service (`Claude Code-credentials`), with MyAgentsBar cache in `com.steipete.myagentsbar.cache` (`oauth.claude`). |
| Migration runs in `MyAgentsBarApp.init()` | Migration runs in `HiddenWindowView` `.task` via detached task (`KeychainMigration.migrateIfNeeded()`). |
...
### 1. Legacy MyAgentsBar keychain migration (V1)
`Sources/MyAgentsBar/KeychainMigration.swift` migrates legacy `com.steipete.MyAgentsBar` items (for example
`claude-cookie`) to `AfterFirstUnlockThisDeviceOnly`.

- Gate key: `KeychainMigrationV1Completed`
- Runs once unless flag is reset.
- Covers legacy MyAgentsBar-managed accounts only (not Claude CLI's own keychain service).

### 2. Claude OAuth bootstrap path
`Sources/MyAgentsBarCore/Providers/Claude/ClaudeOAuth/ClaudeOAuthCredentials.swift`

Load order for credentials:
1. Environment override (`CODEXBAR_CLAUDE_OAUTH_TOKEN`, scopes env key).
2. In-memory cache.
3. MyAgentsBar keychain cache (`com.steipete.myagentsbar.cache`, account `oauth.claude`).
...
When MyAgentsBar does not have usable OAuth credentials in its own cache (`com.steipete.myagentsbar.cache` / `oauth.claude`),
...
`Sources/MyAgentsBarCore/CookieHeaderCache.swift` and `Sources/MyAgentsBarCore/KeychainCacheStore.swift`

- Browser-imported Claude session cookies are cached in keychain service `com.steipete.myagentsbar.cache`.
...
- Keychain cache store (`com.steipete.myagentsbar.cache`) also writes with `ThisDeviceOnly`.
...
```bash
defaults read com.steipete.myagentsbar KeychainMigrationV1Completed
```

### Check Claude OAuth keychain cooldown
```bash
defaults read com.steipete.myagentsbar claudeOAuthKeychainDeniedUntil
```

### Inspect keychain-related logs
```bash
log show --predicate 'subsystem == "com.steipete.myagentsbar" && (category == "keychain-migration" || category == "keychain-preflight" || category == "keychain-prompt" || category == "keychain-cache" || category == "claude-usage" || category == "cookie-cache")' --last 10m
```

### Reset migration for local testing
```bash
defaults delete com.steipete.myagentsbar KeychainMigrationV1Completed
./Scripts/compile_and_run.sh
```

## Key files (current)

- `Sources/MyAgentsBar/KeychainMigration.swift`
- `Sources/MyAgentsBar/HiddenWindowView.swift`
- `Sources/MyAgentsBarCore/Providers/Claude/ClaudeOAuth/ClaudeOAuthCredentials.swift`
- `Sources/MyAgentsBarCore/Providers/Claude/ClaudeOAuth/ClaudeOAuthKeychainAccessGate.swift`
- `Sources/MyAgentsBarCore/KeychainAccessPreflight.swift`
- `Sources/MyAgentsBarCore/KeychainNoUIQuery.swift`
- `Sources/MyAgentsBarCore/KeychainCacheStore.swift`
- `Sources/MyAgentsBarCore/CookieHeaderCache.swift`
