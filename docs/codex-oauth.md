...
> Replicate Codex's direct OAuth token usage in MyAgentsBar instead of calling the CLI.

## Background

Currently, MyAgentsBar fetches Codex usage by:
...
- Command: `MyAgentsBarCLI usage --provider codex --source oauth --json --pretty`
...
|------|----------|---------|
| `CodexOAuthCredentials.swift` | `Sources/MyAgentsBarCore/Providers/Codex/CodexOAuth/` | Token storage model + loader |
| `CodexOAuthUsageFetcher.swift` | `Sources/MyAgentsBarCore/Providers/Codex/CodexOAuth/` | API client for usage endpoint |
| `CodexTokenRefresher.swift` | `Sources/MyAgentsBarCore/Providers/Codex/CodexOAuth/` | Token refresh logic |
...
public enum CodexOAuthUsageFetcher {
    ...
    public static func fetchUsage(
        ...
        request.setValue("MyAgentsBar", forHTTPHeaderField: "User-Agent")
...
2. Run MyAgentsBar with debug logging enabled
