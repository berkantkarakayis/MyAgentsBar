...
1) **API token** (preferred)
   - Set in Preferences → Providers → MiniMax (stored in `~/.myagentsbar/config.json`) or `MINIMAX_API_KEY`.
...
2) **Cached cookie header** (automatic, only when no API token)
   - Keychain cache: `com.steipete.myagentsbar.cache` (account `cookie.minimax`).
...
5) **Manual session cookie header** (optional override)
   - Stored in `~/.myagentsbar/config.json` via Preferences → Providers → MiniMax (Cookie source → Manual).
...
## Key files
- `Sources/MyAgentsBarCore/Providers/MiniMax/MiniMaxUsageFetcher.swift`
- `Sources/MyAgentsBarCore/Providers/MiniMax/MiniMaxProviderDescriptor.swift`
- `Sources/MyAgentsBar/Providers/MiniMax/MiniMaxProviderImplementation.swift`
