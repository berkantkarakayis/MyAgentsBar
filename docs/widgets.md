---
summary: "WidgetKit snapshot pipeline + visibility troubleshooting for MyAgentsBar widgets."
read_when:
  - Modifying WidgetKit extension behavior or snapshot format
  - Debugging widget update timing
  - Widget gallery shows no MyAgentsBar widgets
---
...
## Extension
- `Sources/MyAgentsBarWidget` contains timeline + views.
...
WAPPEX="$APP/Contents/PlugIns/MyAgentsBarWidget.appex"
...
```
pluginkit -m -p com.apple.widgetkit-extension -v | grep -i myagentsbar || true
pluginkit -m -p com.apple.widgetkit-extension -i com.steipete.myagentsbar.widget -vv
```
Notes:
...
```
pluginkit -a "$WAPPEX"
pluginkit -e use -p com.apple.widgetkit-extension -i com.steipete.myagentsbar.widget
```
- Check for duplicates (old installs or version precedence):
```
pluginkit -m -D -p com.apple.widgetkit-extension -i com.steipete.myagentsbar.widget -vv
```
...
```
codesign --verify --deep --strict --verbose=4 /Applications/MyAgentsBar.app
codesign --verify --strict --verbose=4 "$WAPPEX"
codesign --verify --strict --verbose=4 "$WAPPEX/Contents/MacOS/MyAgentsBarWidget"
spctl --assess --type execute --verbose=4 /Applications/MyAgentsBar.app
```
...
- Widget bundle id should be `com.steipete.myagentsbar.widget`.
- `NSExtensionPointIdentifier` must be `com.apple.widgetkit-extension`.
- Bundle folder name should match: `MyAgentsBarWidget.appex`.
