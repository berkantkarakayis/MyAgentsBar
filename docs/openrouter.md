...
You can also configure the API key in MyAgentsBar Settings → Providers → OpenRouter.
...
```bash
myagentsbar --provider openrouter
myagentsbar -p or  # alias
```
...
| `OPENROUTER_X_TITLE` | Optional client title sent as `X-Title` header (defaults to `MyAgentsBar`) |
