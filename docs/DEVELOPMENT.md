...
1. **Make code changes** in `Sources/MyAgentsBar/`
2. **Run** `./Scripts/compile_and_run.sh` to rebuild and launch
3. **Check logs** in Console.app (filter by "myagentsbar")
4. **Optional file log**: enable Debug → Logging → "Enable file logging" to write
   `~/Library/Logs/MyAgentsBar/MyAgentsBar.log` (verbosity defaults to "Verbose")
...
### How It Works
MyAgentsBar automatically refreshes Augment cookies from your browser:

1. **Automatic Import**: On every usage refresh, MyAgentsBar imports fresh cookies from your browser
...
```
MyAgentsBar/
├── Sources/MyAgentsBar/          # Main app (SwiftUI + AppKit)
│   ├── MyAgentsBarApp.swift      # App entry point
...
├── Sources/MyAgentsBarCore/      # Shared business logic
├── Tests/MyAgentsBarTests/       # XCTest suite
...
### Add a New Provider
1. Create `Sources/MyAgentsBar/Providers/YourProvider/`
...
# Filter: subsystem:com.steipete.myagentsbar category:augment-cookie
```
...
```bash
./Scripts/package_app.sh
# Creates: MyAgentsBar.app (ad-hoc signed)
```

### Release Build (Notarized)
```bash
./Scripts/sign-and-notarize.sh
# Creates: MyAgentsBar-arm64.zip (notarized for distribution)
```
...
```bash
# Check crash logs
ls -lt ~/Library/Logs/DiagnosticReports/MyAgentsBar* | head -5

# Check Console.app for errors
# Filter: process:MyAgentsBar
```
