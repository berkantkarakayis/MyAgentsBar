...
1) **Cached cookie header** (Keychain cache `com.steipete.myagentsbar.cache`, account `cookie.factory`).
2) **Stored session** (`~/Library/Application Support/MyAgentsBar/factory-session.json`).
...
## Session storage
- File: `~/Library/Application Support/MyAgentsBar/factory-session.json`
...
## Key files
- `Sources/MyAgentsBarCore/Providers/Factory/FactoryStatusProbe.swift`
- `Sources/MyAgentsBarCore/Providers/Factory/FactoryLocalStorageImporter.swift`
