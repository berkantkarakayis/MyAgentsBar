...
- `Sources/MyAgentsBarCore`: provider descriptors + fetch strategies + probes + parsing + shared utilities.
- `Sources/MyAgentsBar`: UI/state + provider implementations (settings/login/menu hooks only).
...
- `Sources/MyAgentsBarCore/Providers/<ProviderID>/`
...
Checklist:
- Add `UsageProvider` case in `Sources/MyAgentsBarCore/Providers/Providers.swift`.
- Create `Sources/MyAgentsBarCore/Providers/<ProviderID>/`:
...
- Add `Sources/MyAgentsBar/Providers/<ProviderID>/<ProviderID>ProviderImplementation.swift`:
