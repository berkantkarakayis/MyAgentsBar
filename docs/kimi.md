...
Tracks usage for [Kimi For Coding](https://www.kimi.com/code) in MyAgentsBar.
...
1. Open MyAgentsBar settings → Providers → Kimi
2. Set "Cookie source" to "Automatic"
3. Enable the Kimi provider toggle
4. MyAgentsBar will automatically find your session

**Note**: Requires Full Disk Access to read browser cookies (System Settings → Privacy & Security → Full Disk Access → MyAgentsBar).
...
7. Paste it into the "Auth Token" field in MyAgentsBar
...
When multiple sources are available, MyAgentsBar uses this order:
...
- Grant Full Disk Access to MyAgentsBar in System Settings
...
- **Core files**: `Sources/MyAgentsBarCore/Providers/Kimi/`
- **UI files**: `Sources/MyAgentsBar/Providers/Kimi/`
- **Login flow**: `Sources/MyAgentsBar/KimiLoginRunner.swift`
- **Tests**: `Tests/MyAgentsBarTests/KimiProviderTests.swift`
