...
**Fork:** topoffunnel/MyAgentsBar  
**Upstream 1:** steipete/MyAgentsBar (original)  
...
```bash
# Your fork (origin)
git remote add origin git@github.com:topoffunnel/MyAgentsBar.git

# Original upstream (steipete)
git remote add upstream git@github.com:steipete/MyAgentsBar.git
  ...
  check-upstream:
    ...
    steps:
      ...
      - name: Add upstream remotes
        run: |
          git remote add upstream https://github.com/steipete/MyAgentsBar.git
      ...
      - name: Create issue if changes detected
        ...
        with:
          script: |
            ...
            **steipete/MyAgentsBar:** ${upstreamCommits} new commits
            **quotio:** ${quotioCommits} new commits (last week)
            
            Review changes:
            - [steipete commits](https://github.com/steipete/MyAgentsBar/compare/main...upstream/main)
...
Implemented independently with MyAgentsBar-specific patterns."
```
...
echo "5. Create PR to steipete/MyAgentsBar from GitHub UI"
```
...
# Create PR to steipete/MyAgentsBar via GitHub UI
```
...
Implemented independently using MyAgentsBar patterns.
```
...
- `Sources/MyAgentsBar/About.swift` (your attribution)
- `Sources/MyAgentsBar/PreferencesAboutPane.swift` (fork sections)
...
# Push and create PR
git push origin upstream-pr/fix-cursor-bonus
# Then create PR on GitHub to steipete/MyAgentsBar
```
...
Implemented independently using MyAgentsBar's architecture."
```
...
# CONFLICT in Sources/MyAgentsBar/About.swift

# Keep your fork version for branding files
git checkout --ours Sources/MyAgentsBar/About.swift
git add Sources/MyAgentsBar/About.swift
