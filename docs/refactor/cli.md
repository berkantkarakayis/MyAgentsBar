...
read_when:
  - "Refactoring MyAgentsBar CLI error handling or config parsing."
...
## CLI commands
- `myagentsbar config validate`
  - Prints JSON issues (or text summary).
  - Exit non-zero if any errors.
- (Optional) `myagentsbar config dump`
  - Prints normalized config JSON.

## Step-by-step implementation guide
1. **Add validation types**
   - `MyAgentsBarConfigIssue` + `MyAgentsBarConfigValidator` in `MyAgentsBarCore/Config`.
   - Keep file <500 LOC.
...
8. **Verification**
   ...
   - CLI e2e: `myagentsbar --json-only ...`, `myagentsbar config validate`.
