...
final class UsageStore {
    ...
    @ObservationIgnored private let sessionQuotaLogger = MyAgentsBarLog.logger(LogCategories.sessionQuota)
    @ObservationIgnored let openAIWebLogger = MyAgentsBarLog.logger(LogCategories.openAIWeb)
    @ObservationIgnored private let tokenCostLogger = MyAgentsBarLog.logger(LogCategories.tokenCost)
    @ObservationIgnored let augmentLogger = MyAgentsBarLog.logger(LogCategories.augment)
    @ObservationIgnored let providerLogger = MyAgentsBarLog.logger(LogCategories.providers)
...
extension UsageStore {
    func debugDumpClaude() async {
        ...
        let url = URL(fileURLWithPath: NSTemporaryDirectory()).appendingPathComponent("myagentsbar-claude-probe.txt")
    ...
    func dumpLog(toFileFor provider: UsageProvider) async -> URL? {
        ...
        let filename = "myagentsbar-\(provider.rawValue)-probe.txt"
    ...
    func debugLog(for provider: UsageProvider) async -> String {
        ...
        let text = await Task.detached(priority: .utility) { () -> String in
            ...
            let buildText = {
                ...
                default:
                    return "Debug log not implemented for \(provider.rawValue)"
                }
            }
            return await claudeDebugExecutionContext.apply {
                await buildText()
        ...
        fallbackCookieHeader: String) async -> ClaudeDebugLogConfiguration
    {
        await MainActor.run {
            ...
            return ClaudeDebugLogConfiguration(
                runtime: MyAgentsBarCore.ProviderRuntime.app,
