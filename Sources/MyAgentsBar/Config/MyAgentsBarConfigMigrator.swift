...
struct MyAgentsBarConfigMigrator {
    struct LegacyStores {
    ...
    static func loadOrMigrate(
        configStore: MyAgentsBarConfigStore,
        userDefaults: UserDefaults,
        stores: LegacyStores) -> MyAgentsBarConfig
    {
        let log = MyAgentsBarLog.logger(LogCategories.configMigration)
        let existing = try? configStore.load()
        var config = (existing ?? MyAgentsBarConfig.makeDefault()).normalized()
    ...
    private static func applyLegacyOrderAndToggles(
        userDefaults: UserDefaults,
        config: inout MyAgentsBarConfig,
    ...
    private static func applyProviderOrder(_ raw: [String], config: MyAgentsBarConfig) -> MyAgentsBarConfig {
    ...
    private static func applyProviderToggles(
        ...
        config: MyAgentsBarConfig) -> MyAgentsBarConfig
    {
        var updated = config
