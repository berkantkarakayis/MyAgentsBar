...
final class SettingsStore {
    static let sharedDefaults = UserDefaults(suiteName: "group.com.steipete.myagentsbar")
    ...
    @ObservationIgnored let configStore: MyAgentsBarConfigStore
    @ObservationIgnored var config: MyAgentsBarConfig
    ...
    init(
        ...
        configStore: MyAgentsBarConfigStore = MyAgentsBarConfigStore(),
        ...
        tokenAccountStore: any ProviderTokenAccountStoring = FileTokenAccountStore())
    {
        ...
        let legacyStores = MyAgentsBarConfigMigrator.LegacyStores(
        ...
        let config = MyAgentsBarConfigMigrator.loadOrMigrate(
        ...
        MyAgentsBarLog.setFileLoggingEnabled(self.debugFileLoggingEnabled)
...
extension SettingsStore {
    ...
    private static func loadDefaultsState(userDefaults: UserDefaults) -> SettingsDefaultsState {
        ...
        let debugLogLevelRaw = userDefaults.string(forKey: "debugLogLevel") ?? MyAgentsBarLog.Level.verbose.rawValue
           ...
           let legacyPreference = MenuBarMetricPreference(rawValue: menuBarMetricRaw)
        {
            resolvedPreferences = Dictionary(
                uniqueKeysWithValues: SupportedProviders.list.map { ($0.rawValue, legacyPreference.rawValue) })
        }
...
extension SettingsStore {
    var configSnapshot: MyAgentsBarConfig {
        _ = self.configRevision
        return self.config
    }

    func updateProviderState(config: MyAgentsBarConfig) {
        ...
        enablement.reserveCapacity(SupportedProviders.list.count)
    ...
    func isProviderEnabled(provider: UsageProvider, metadata: ProviderMetadata) -> Bool {
        guard SupportedProviders.contains(provider) else { return false }
    ...
    func setProviderEnabled(provider: UsageProvider, metadata _: ProviderMetadata, enabled: Bool) {
        ...
        MyAgentsBarLog.logger(LogCategories.settings).debug(
...
extension SettingsStore {
    private static func effectiveProviderOrder(raw: [String]) -> [UsageProvider] {
        ...
        if ordered.isEmpty {
            ordered = SupportedProviders.list
            seen = Set(ordered)
        }

        for provider in SupportedProviders.list where !seen.contains(provider) {
            ordered.append(provider)
        }
