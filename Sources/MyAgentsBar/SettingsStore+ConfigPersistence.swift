...
extension SettingsStore {
    private func updateConfig(reason: String, mutate: (inout MyAgentsBarConfig) -> Void) {
    ...
    func updateProviderTokenAccounts(_ accounts: [UsageProvider: ProviderTokenAccountData]) {
        let summary = accounts
            ...
            .joined(separator: ",")
        MyAgentsBarLog.logger(LogCategories.tokenAccounts).info(
    ...
    func reloadConfig(reason: String) {
        ...
        do {
            ...
            MyAgentsBarLog.logger(LogCategories.configStore).error("Failed to reload config: \(error)")
        }
    }

    func applyExternalConfig(_ config: MyAgentsBarConfig, reason: String) {
        guard !self.configLoading else { return }
    ...
    func schedulePersistConfig() {
        ...
        if Self.isRunningTests {
            do {
                ...
                MyAgentsBarLog.logger(LogCategories.configStore).error("Failed to persist config: \(error)")
            }
            return
        }
