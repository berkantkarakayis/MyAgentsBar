...
final class OpenAICreditsPurchaseWindowController: NSWindowController, WKNavigationDelegate, WKScriptMessageHandler {
    ...
    private static let logHandlerName = "myagentsbarLog"
    private static let debugLogURL = URL(fileURLWithPath: NSTemporaryDirectory())
        .appendingPathComponent("myagentsbar-buy-credits.log")
    private static let autoStartScript = """
    (() => {
      if (window.__myagentsbarAutoBuyCreditsStarted) return 'already';
      const log = (...args) => {
        try {
          window.webkit?.messageHandlers?.myagentsbarLog?.postMessage(args);
        } catch {}
      };
      ...
      const startNextPolling = (initialDelay = 500, interval = 500, maxAttempts = 90) => {
        if (window.__myagentsbarNextPolling) return;
        window.__myagentsbarNextPolling = true;
      ...
      const observeNextButton = () => {
        if (window.__myagentsbarNextObserver || !window.MutationObserver) return;
        const observer = new MutationObserver(() => {
          if (clickNextIfReady(1)) {
            observer.disconnect();
            window.__myagentsbarNextObserver = null;
          }
        });
        observer.observe(document.body, { subtree: true, childList: true, attributes: true });
        window.__myagentsbarNextObserver = observer;
      };
      ...
      if (findAndClick()) {
        window.__myagentsbarAutoBuyCreditsStarted = true;
        startNextPolling();
        observeNextButton();
        logDialogButtons();
        return 'clicked';
    ...
    private let logger = MyAgentsBarLog.logger(LogCategories.creditsPurchase)
