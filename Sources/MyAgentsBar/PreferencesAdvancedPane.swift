        ...
        ScrollView(.vertical, showsIndicators: true) {
            VStack(alignment: .leading, spacing: 16) {
                ...
                SettingsSection(contentSpacing: 10) {
                    ...
                    Text("Symlink MyAgentsBarCLI to /usr/local/bin and /opt/homebrew/bin as myagentsbar.")
                        .font(.footnote)
                        .foregroundStyle(.tertiary)
                }
...
extension AdvancedPane {
    private func installCLI() async {
        ...
        let helperURL = Bundle.main.bundleURL.appendingPathComponent("Contents/Helpers/MyAgentsBarCLI")
        let fm = FileManager.default
        guard fm.fileExists(atPath: helperURL.path) else {
            self.cliStatus = "MyAgentsBarCLI not found in app bundle."
            return
        }

        let destinations = [
            "/usr/local/bin/myagentsbar",
            "/opt/homebrew/bin/myagentsbar",
        ]
