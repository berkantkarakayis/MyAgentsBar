...
struct MyAgentsBarApp: App {
    ...
    init() {
        ...
        let storedLevel = MyAgentsBarLog.parseLevel(UserDefaults.standard.string(forKey: "debugLogLevel")) ?? .verbose
        let level = MyAgentsBarLog.parseLevel(env["CODEXBAR_LOG_LEVEL"]) ?? storedLevel
        MyAgentsBarLog.bootstrapIfNeeded(.init(
            destination: .oslog(subsystem: "com.steipete.myagentsbar"),
            level: level,
            json: false))
        ...
        MyAgentsBarLog.logger(LogCategories.app).info(
            "MyAgentsBar starting",
            metadata: [
                "version": version,
                "build": build,
                "git": gitCommit,
        ...
        MyAgentsBarLog.setLogLevel(settings.debugLogLevel)
    ...
    var body: some Scene {
        ...
        WindowGroup("MyAgentsBarLifecycleKeepalive") {
            HiddenWindowView()
        }
...
private func makeUpdaterController() -> UpdaterProviding {
    ...
    if InstallOrigin.isHomebrewCask(appBundleURL: bundleURL) {
        return DisabledUpdaterController(
            unavailableReason: "Updates managed by Homebrew. Run: brew upgrade --cask steipete/tap/myagentsbar")
    }
...
final class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    private func ensureStatusController() {
        ...
        MyAgentsBarLog.logger(LogCategories.app)
