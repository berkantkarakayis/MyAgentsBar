...
struct MyAgentsBarWidgetEntry: TimelineEntry {
    let date: Date
    let provider: UsageProvider
    let snapshot: WidgetSnapshot
}

struct MyAgentsBarCompactEntry: TimelineEntry {
    let date: Date
    let provider: UsageProvider
...
struct MyAgentsBarSwitcherEntry: TimelineEntry {
    let date: Date
    let provider: UsageProvider
...
struct MyAgentsBarTimelineProvider: AppIntentTimelineProvider {
    func placeholder(in context: Context) -> MyAgentsBarWidgetEntry {
        MyAgentsBarWidgetEntry(
            date: Date(),
            provider: .codex,
            snapshot: WidgetPreviewData.snapshot())
    }

    func snapshot(for configuration: ProviderSelectionIntent, in context: Context) async -> MyAgentsBarWidgetEntry {
        let provider = configuration.provider.provider
        return MyAgentsBarWidgetEntry(
            date: Date(),
            provider: provider,
    ...
    func timeline(
        ...
        in context: Context) async -> Timeline<MyAgentsBarWidgetEntry>
    {
        let provider = configuration.provider.provider
        let snapshot = WidgetSnapshotStore.load() ?? WidgetPreviewData.snapshot()
        let entry = MyAgentsBarWidgetEntry(date: Date(), provider: provider, snapshot: snapshot)
        let refresh = Date().addingTimeInterval(30 * 60)
        return Timeline(entries: [entry], policy: .after(refresh))
    }
}

struct MyAgentsBarSwitcherTimelineProvider: TimelineProvider {
    func placeholder(in context: Context) -> MyAgentsBarSwitcherEntry {
        let snapshot = WidgetPreviewData.snapshot()
        let providers = self.availableProviders(from: snapshot)
        return MyAgentsBarSwitcherEntry(
            date: Date(),
    ...
    func getSnapshot(in context: Context, completion: @escaping (MyAgentsBarSwitcherEntry) -> Void) {
        completion(self.makeEntry())
    }

    func getTimeline(in context: Context, completion: @escaping (Timeline<MyAgentsBarSwitcherEntry>) -> Void) {
        let entry = self.makeEntry()
        let refresh = Date().addingTimeInterval(30 * 60)
        completion(Timeline(entries: [entry], policy: .after(refresh)))
    }

    private func makeEntry() -> MyAgentsBarSwitcherEntry {
        let snapshot = WidgetSnapshotStore.load() ?? WidgetPreviewData.snapshot()
        let providers = self.availableProviders(from: snapshot)
...
struct MyAgentsBarCompactTimelineProvider: AppIntentTimelineProvider {
    func placeholder(in context: Context) -> MyAgentsBarCompactEntry {
        MyAgentsBarCompactEntry(
            date: Date(),
            provider: .codex,
            metric: .credits,
            snapshot: WidgetPreviewData.snapshot())
    ...
    func snapshot(for configuration: CompactMetricSelectionIntent, in context: Context) async -> MyAgentsBarCompactEntry {
        let provider = configuration.provider.provider
        return MyAgentsBarCompactEntry(
            date: Date(),
            provider: provider,
    ...
    func timeline(
        ...
        in context: Context) async -> Timeline<MyAgentsBarCompactEntry>
    {
        let provider = configuration.provider.provider
        let snapshot = WidgetSnapshotStore.load() ?? WidgetPreviewData.snapshot()
        let entry = MyAgentsBarCompactEntry(
            date: Date(),
            provider: provider,
            metric: configuration.metric,
            snapshot: snapshot)
