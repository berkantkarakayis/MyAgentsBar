...
enum MyAgentsBarCLI {
    ...
    private static func bootstrapLogging(values: ParsedValues) {
        ...
        MyAgentsBarLog.bootstrapIfNeeded(.init(destination: .stderr, level: level, json: isJSON))
    }

    static func resolvedLogLevel(verbose: Bool, rawLevel: String?) -> MyAgentsBarLog.Level {
        MyAgentsBarLog.parseLevel(rawLevel) ?? (verbose ? .debug : .error)
    }
