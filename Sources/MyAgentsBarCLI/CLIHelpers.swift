...
extension MyAgentsBarCLI {
    static func decodeProvider(from values: ParsedValues, config: MyAgentsBarConfig) -> ProviderSelection {
    ...
    static func resetTimeDisplayStyleFromDefaults() -> ResetTimeDisplayStyle {
        let domains = [
            "com.steipete.myagentsbar",
            "com.steipete.myagentsbar.debug",
        ]
    ...
    static func loadConfig(output: CLIOutputPreferences) -> MyAgentsBarConfig {
        let store = MyAgentsBarConfigStore()
        do {
            if let existing = try store.load() {
                return existing
            }
            return MyAgentsBarConfig.makeDefault()
        } catch {
