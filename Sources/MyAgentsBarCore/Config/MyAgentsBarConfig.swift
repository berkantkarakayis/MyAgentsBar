...
public struct MyAgentsBarConfig: Codable, Sendable {
    ...
    public static func makeDefault(
        metadata: [UsageProvider: ProviderMetadata] = ProviderDescriptorRegistry.metadata) -> MyAgentsBarConfig
    {
        let providers = SupportedProviders.list.map { provider in
            ProviderConfig(
                id: provider,
                enabled: metadata[provider]?.defaultEnabled)
        }
        return MyAgentsBarConfig(version: Self.currentVersion, providers: providers)
    }
        ...
        metadata: [UsageProvider: ProviderMetadata] = ProviderDescriptorRegistry.metadata) -> MyAgentsBarConfig
    {
        ...
        normalized.reserveCapacity(max(self.providers.count, SupportedProviders.list.count))
