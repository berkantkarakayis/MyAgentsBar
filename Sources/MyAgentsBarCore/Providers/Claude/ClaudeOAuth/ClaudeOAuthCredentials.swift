...
public enum ClaudeOAuthCredentialsStore {
    ...
    static let log = MyAgentsBarLog.logger(LogCategories.claudeUsage)
    ...
    /// - MyAgentsBar-owned credentials refresh directly via token endpoint.
    ...
    /// Save refreshed credentials to MyAgentsBar's keychain cache
    private static func saveRefreshedCredentialsToCache(_ credentials: ClaudeOAuthCredentials) {
        ...
        self.saveToCacheKeychain(jsonData, owner: .myagentsbar)
        self.log.debug("Saved refreshed credentials to MyAgentsBar keychain cache")
    }
    ...
    /// Check if MyAgentsBar has cached credentials (in memory or keychain cache)
...
extension ClaudeOAuthCredentialsStore {
    ...
    /// MyAgentsBar's caches. This is used to avoid triggering a second OS keychain dialog during the OAuth retry.
    @discardableResult
    ...
    /// Updates MyAgentsBar's keychain cache with the new credentials.
