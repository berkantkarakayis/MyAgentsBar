...
public enum CursorStatusProbeError: LocalizedError, Sendable {
    ...
    public var errorDescription: String? {
        switch self {
        case .notLoggedIn:
            "Not logged in to Cursor. Please log in via the MyAgentsBar menu."
        ...
        case .noSessionCookie:
            "No Cursor session found. Please log in to cursor.com in \(cursorCookieImportOrder.loginHint). "
                + "If you use Safari, grant MyAgentsBar Full Disk Access in System Settings ▸ Privacy & Security. "
                + "You can also sign in to Cursor from the MyAgentsBar menu (Add / switch account)."
        }
    }
}
...
public actor CursorSessionStore {
    ...
    private init() {
        ...
        let dir = appSupport.appendingPathComponent("MyAgentsBar", isDirectory: true)
