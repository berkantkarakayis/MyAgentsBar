...
private final class CodexRPCClient: @unchecked Sendable {
    private static let log = MyAgentsBarLog.logger(LogCategories.codexRPC)
        ...
        environment: [String: String] = ProcessInfo.processInfo.environment) throws
    {
        ...
        guard let resolvedExec else {
            ...
            throw RPCWireError.startFailed(
                "Codex CLI not found. Install with `npm i -g @openai/codex` (or bun) then relaunch MyAgentsBar.")
        }
        var env = environment
...
public struct UsageFetcher: Sendable {
    ...
    private func loadRPCUsage() async throws -> UsageSnapshot {
        ...
        defer { rpc.shutdown() }
        do {
            try await rpc.initialize(clientName: "myagentsbar", clientVersion: "0.5.4")
