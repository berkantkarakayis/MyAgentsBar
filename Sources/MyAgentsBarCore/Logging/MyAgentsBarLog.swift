...
public enum MyAgentsBarLog {
    ...
    public static func logger(_ category: String) -> MyAgentsBarLogger {
        let logger = Logger(label: "com.steipete.myagentsbar.\(category)")
        return MyAgentsBarLogger { level, message, metadata in
...
public struct MyAgentsBarLogger: Sendable {
    private let logFn: @Sendable (MyAgentsBarLog.Level, String, [String: String]?) -> Void

    fileprivate init(_ logFn: @escaping @Sendable (MyAgentsBarLog.Level, String, [String: String]?) -> Void) {
        self.logFn = logFn
    }
