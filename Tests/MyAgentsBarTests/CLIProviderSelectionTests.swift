...
struct CLIProviderSelectionTests {
    ...
    func `help includes gemini and all`() {
        let usage = MyAgentsBarCLI.usageHelp(version: "0.0.0")
        let root = MyAgentsBarCLI.rootHelp(version: "0.0.0")
        ...
        #expect(usage.contains("myagentsbar usage --provider gemini"))
        #expect(usage.contains("myagentsbar usage --format json --provider all --pretty"))
        #expect(root.contains("myagentsbar --provider gemini"))
    }
    ...
    func `provider selection respects override`() {
        let selection = MyAgentsBarCLI.providerSelection(rawOverride: "gemini", enabled: [.codex, .claude])
        #expect(selection.asList == [.gemini])
    }

    @Test
    func `provider selection uses all when enabled`() {
        let selection = MyAgentsBarCLI.providerSelection(
            rawOverride: nil,
    ...
    func `provider selection uses custom for codex and gemini`() {
        ...
        let selection = MyAgentsBarCLI.providerSelection(rawOverride: nil, enabled: enabled)
        #expect(selection.asList == enabled)
    }

    @Test
    func `provider selection accepts kiro alias`() {
        let selection = MyAgentsBarCLI.providerSelection(rawOverride: "kiro-cli", enabled: [.codex])
