...
struct CLIArgumentParsingTests {
    ...
    func `json shortcut does not enable json logs`() throws {
        ...
        #expect(MyAgentsBarCLI._decodeFormatForTesting(from: parsed) == .json)
    }
    ...
    func `json output flag enables json logs`() throws {
        ...
        #expect(parsed.flags.contains("jsonOutput"))
        #expect(!parsed.flags.contains("jsonShortcut"))
        #expect(MyAgentsBarCLI._decodeFormatForTesting(from: parsed) == .text)
    }
    ...
    func `resolved log level defaults to error`() {
        #expect(MyAgentsBarCLI.resolvedLogLevel(verbose: false, rawLevel: nil) == .error)
        #expect(MyAgentsBarCLI.resolvedLogLevel(verbose: true, rawLevel: nil) == .debug)
        #expect(MyAgentsBarCLI.resolvedLogLevel(verbose: false, rawLevel: "info") == .info)
    }
