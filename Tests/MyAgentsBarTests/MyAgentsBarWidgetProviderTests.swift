...
@testable import MyAgentsBarWidget

struct MyAgentsBarWidgetProviderTests {
    @Test
    ...
    func `supported providers keep alibaba when it is the only enabled provider`() {
        ...
        #expect(MyAgentsBarSwitcherTimelineProvider.supportedProviders(from: snapshot) == [.alibaba])
    }
