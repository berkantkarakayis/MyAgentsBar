...
- Build script: make MyAgentsBar-owned ad-hoc keychain cleanup opt-in with `--clear-adhoc-keychain`, and extend the explicit reset path to clear both `com.steipete.MyAgentsBar` and `com.steipete.myagentsbar.cache`. Thanks @magnaprog!
...
### Claude OAuth & Keychain
- Claude OAuth creds are cached in MyAgentsBar Keychain to reduce repeated prompts.
...
- CLI/App: move provider secrets + ordering to `~/.myagentsbar/config.json` (no Keychain persistence).
...
- CLI: add `myagentsbar cost` to print local cost usage (text/JSON) for Codex + Claude.
...
- macOS: MyAgentsBar now supports Intel Macs (x86_64 builds + Sonoma fallbacks). Thanks @epoyraz!
...
- Homebrew: cask now installs the `myagentsbar` CLI symlink. Thanks @dalisoft!
...
- Widgets: add a new “MyAgentsBar Switcher” widget that lets you switch providers and remember the selection.
...
- CI: build/test Linux `MyAgentsBarCLI` (x86_64 + aarch64) and publish release assets as `MyAgentsBarCLI-<tag>-linux-<arch>.tar.gz` (+ `.sha256`).
...
## 0.8.0 — 2025-12-17
- MyAgentsBar is now available via Homebrew: `brew install --cask steipete/tap/myagentsbar` (updates via `brew upgrade --cask steipete/tap/myagentsbar`).
...
## 0.6.0 — 2025-12-04
- New bundled CLI (`myagentsbar`) with single `usage` command, `--format text|json`, `--status`, and fast `-h/-V`.
- CLI output now shows consistent headers (`Codex 0.x.y (codex-cli)`, `Claude Code <ver> (claude)`) and JSON includes `source` + `status`.
- Advanced prefs install button symlinks `myagentsbar` into /usr/local/bin and /opt/homebrew/bin; docs refreshed.
...
- Debug replay toggle (`defaults write com.steipete.myagentsbar debugMenuEnabled -bool YES`) to view every pattern.
...
- Notarized release build "MyAgentsBar-0.1.0.zip" remains current artifact; app version 0.1.1.

## 0.1.0 — 2025-11-16
- Initial MyAgentsBar release: macOS 15+ menu bar app, no Dock icon.
