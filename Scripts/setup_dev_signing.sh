...
# Check if we already have a MyAgentsBar development certificate
CERT_NAME="MyAgentsBar Development"
...
openssl req -x509 -newkey rsa:4096 -sha256 -days 3650 \
    -nodes -keyout /tmp/myagentsbar-dev.key -out /tmp/myagentsbar-dev.crt \
    -config "$TEMP_CONFIG" 2>/dev/null

# Convert to PKCS12 format
openssl pkcs12 -export -out /tmp/myagentsbar-dev.p12 \
    -inkey /tmp/myagentsbar-dev.key -in /tmp/myagentsbar-dev.crt \
    -passout pass: 2>/dev/null

# Import into keychain
security import /tmp/myagentsbar-dev.p12 -k ~/Library/Keychains/login.keychain-db -T /usr/bin/codesign -T /usr/bin/security

# Clean up temporary files
rm -f /tmp/myagentsbar-dev.{key,crt,p12}
