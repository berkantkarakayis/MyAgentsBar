...
APP_NAME="MyAgentsBar"
APP_IDENTITY="Developer ID Application: Berkant Karakayış"
APP_BUNDLE="MyAgentsBar.app"
...
echo "$APP_STORE_CONNECT_API_KEY_P8" | sed 's/\\n/\n/g' > /tmp/myagentsbar-api-key.p8
trap 'rm -f /tmp/myagentsbar-api-key.p8 /tmp/${APP_NAME}Notarize.zip' EXIT
...
if [[ -f "$APP_BUNDLE/Contents/Helpers/MyAgentsBarCLI" ]]; then
  codesign --force --timestamp --options runtime --sign "$APP_IDENTITY" \
    "$APP_BUNDLE/Contents/Helpers/MyAgentsBarCLI"
fi
if [[ -f "$APP_BUNDLE/Contents/Helpers/MyAgentsBarClaudeWatchdog" ]]; then
  codesign --force --timestamp --options runtime --sign "$APP_IDENTITY" \
    "$APP_BUNDLE/Contents/Helpers/MyAgentsBarClaudeWatchdog"
fi
if [[ -d "$APP_BUNDLE/Contents/PlugIns/MyAgentsBarWidget.appex" ]]; then
  codesign --force --timestamp --options runtime --sign "$APP_IDENTITY" \
    --entitlements "$WIDGET_ENTITLEMENTS" \
    "$APP_BUNDLE/Contents/PlugIns/MyAgentsBarWidget.appex/Contents/MacOS/MyAgentsBarWidget"
  codesign --force --timestamp --options runtime --sign "$APP_IDENTITY" \
    --entitlements "$WIDGET_ENTITLEMENTS" \
    "$APP_BUNDLE/Contents/PlugIns/MyAgentsBarWidget.appex"
fi
codesign --force --timestamp --options runtime --sign "$APP_IDENTITY" \
...
xcrun notarytool submit "/tmp/${APP_NAME}Notarize.zip" \
  --key /tmp/myagentsbar-api-key.p8 \
