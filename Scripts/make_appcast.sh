...
ZIP=${1:?
"Usage: $0 MyAgentsBar-<ver>.zip"}
FEED_URL=${2:-"https://raw.githubusercontent.com/steipete/MyAgentsBar/main/appcast.xml"}
...
if [[ -z "$VERSION" ]]; then
  if [[ "$ZIP_NAME" =~ ^MyAgentsBar-([0-9]+(\.[0-9]+){1,2}([-.][^.]*)?)\.zip$ ]]; then
...
DOWNLOAD_URL_PREFIX=${SPARKLE_DOWNLOAD_URL_PREFIX:-"https://github.com/steipete/MyAgentsBar/releases/download/v${VERSION}/"}
...
WORK_DIR=$(mktemp -d /tmp/myagentsbar-appcast.XXXXXX)
