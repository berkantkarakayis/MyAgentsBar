#!/usr/bin/env bash
# Reset MyAgentsBar: kill running instances, build, package, relaunch, verify.

set -euo pipefail

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
APP_BUNDLE="${ROOT_DIR}/MyAgentsBar.app"
APP_PROCESS_PATTERN="MyAgentsBar.app/Contents/MacOS/MyAgentsBar"
DEBUG_PROCESS_PATTERN="${ROOT_DIR}/.build/debug/MyAgentsBar"
RELEASE_PROCESS_PATTERN="${ROOT_DIR}/.build/release/MyAgentsBar"
LOCK_KEY="$(printf '%s' "${ROOT_DIR}" | shasum -a 256 | cut -c1-8)"
LOCK_DIR="${TMPDIR:-/tmp}/myagentsbar-compile-and-run-${LOCK_KEY}"
...
resolve_signing_mode() {
  ...
  local candidate=""
  for candidate in \
    "Developer ID Application: Berkant Karakayış" \
    "MyAgentsBar Development"
  do
...
kill_all_myagentsbar() {
  ...
  for _ in {1..25}; do
    ...
    pkill -x "MyAgentsBar" 2>/dev/null || true
    if ! is_running; then
      return 0
    fi
    sleep 0.2
  ...
  pkill -9 -x "MyAgentsBar" 2>/dev/null || true
  ...
  fail "Failed to kill all MyAgentsBar instances."
}
...
# 2) Kill all running MyAgentsBar instances (debug, release, bundled).
log "==> Killing existing MyAgentsBar instances"
kill_all_myagentsbar
kill_claude_probes
...
if [[ "${SIGNING_MODE:-adhoc}" == "adhoc" && "${CLEAR_ADHOC_KEYCHAIN}" == "1" ]]; then
  log "==> Clearing MyAgentsBar keychain entries (adhoc signing)"
  # Clear both the legacy keychain store and the current cache service when developers explicitly want a clean reset
  # of MyAgentsBar-owned keychain state for ad-hoc builds.
  delete_keychain_service_items "com.steipete.MyAgentsBar"
  delete_keychain_service_items "com.steipete.myagentsbar.cache"
elif [[ "${SIGNING_MODE:-adhoc}" == "adhoc" ]]; then
  log "==> Preserving MyAgentsBar keychain entries (pass --clear-adhoc-keychain to reset adhoc keychain state)"
fi
...
if ! open "${APP_BUNDLE}"; then
  ...
  "${APP_BUNDLE}/Contents/MacOS/MyAgentsBar" >/dev/null 2>&1 &
  disown
fi
...
for _ in {1..10}; do
  if pgrep -f "${APP_PROCESS_PATTERN}" >/dev/null 2>&1 \
    || pgrep -f "${DEBUG_PROCESS_PATTERN}" >/dev/null 2>&1 \
    || pgrep -f "${RELEASE_PROCESS_PATTERN}" >/dev/null 2>&1 \
    || pgrep -x "MyAgentsBar" >/dev/null 2>&1; then
    log "OK: MyAgentsBar is running."
    exit 0
  fi
  sleep 0.4
done
